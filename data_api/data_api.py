...
@app.route('/cart/<int:syllabus_id>', methods=['PUT'])
def update_cart_quantity(syllabus_id):
    from flask import request
    ...
    cart = db.get("cart", [])
    item = next((c for c in cart if c["id"] == syllabus_id), None)
...
def add_order():
    ...
    new_order_id = len(db.get("orders", [])) + 1
